...
const Adams = () => {
    ...
    const deleteAdam = async (adam) => {
        try {
            const response = await fetch(`${url}/gadgets/adam/${adam.id}/`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
                    ...
                    <table className="text-left w-full border-collapse">
                        <thead>
                            <tr>
                                ...
                                <th className="py-4 px-6 bg-grey-lightest font-bold uppercase text-sm text-grey-dark border-b border-grey-light">{t("IP")}</th>
                                <th className="py-4 px-6 bg-grey-lightest font-bold uppercase text-sm text-grey-dark border-b border-grey-light">{t("Actuation Port")}</th>
                        ...
                        <tbody>
                            {currentItems.map((adam, index) => (
                                <tr key={index} className="hover:bg-grey-lighter">
                                    ...
                                    <td className="py-4 px-6 border-b border-grey-light">{adam.actuation_port}</td>
