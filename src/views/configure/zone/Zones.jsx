...
const Zones = () => {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const response = await fetch(`${url}/gadgets/adam-linkedwith/`, {
                method: "GET",
                headers: {
            ...
            <div className="flex justify-between items-center m-6">
                <div className="flex items-center space-x-2">
                    <input
                        ...
                        type="text"
                        placeholder={t("Search Zone Name")}
                        value={searchTerm}
            ...
            {isLoading ? (
                ...
                <div className="bg-white shadow-md rounded my-6">
                    <table className="text-left w-full border-collapse">
                        <thead>
                            <tr>
                                <th className="py-4 px-6 bg-grey-lightest font-bold uppercase text-sm text-grey-dark border-b border-grey-light">{t("ID")}</th>
                                <th className="py-4 px-6 bg-grey-lightest font-bold uppercase text-sm text-grey-dark border-b border-grey-light">{t("Client IP")}</th>
                                <th className="py-4 px-6 bg-grey-lightest font-bold uppercase text-sm text-grey-dark border-b border-grey-light">{t("adam")}</th>
                                <th className="py-4 px-6 bg-grey-lightest font-bold uppercase text-sm text-grey-dark border-b border-grey-light">{t("Name")}</th>
                                <th className="py-4 px-6 bg-grey-lightest font-bold uppercase text-sm text-grey-dark border-b border-grey-light">{t("other")}</th>
                                <th className="py-4 px-6 bg-grey-lightest font-bold uppercase text-sm text-grey-dark border-b border-grey-light">{t("Action")}</th>
                            </tr>
                        </thead>
                        <tbody>
                            {currentItems.map((zone, index) => (
                                <tr key={index} className="hover:bg-grey-lighter">
                                    ...
                                    <td className="py-4 px-6 border-b border-grey-light">{zone.client_ip}</td>
                                    <td className="py-4 px-6 border-b border-grey-light">{zone.adam && zone.adam.name}</td>
                                    <td className="py-4 px-6 border-b border-grey-light">{zone.name}</td>
                                    <td className="py-4 px-6 border-b border-grey-light">{zone.other}</td>
