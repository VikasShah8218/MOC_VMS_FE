...
import MultipleSelectDropdown from './MultipleSelectDropdown.jsx';
...
const AddNewGuardReaderMapping = ({ open, onClose, fetchData }) => {
    ...
    const handelUserChnage = (newSelectedUser) => {
        // setPassData({ ...passData, zones_allowed: newSelectedZones });
        setSelectedUserList(newSelectedUser);
    };
    ...
    const handleSubmit = async () => {
        ...
        if (!validate()) return;
        try {
            const response = await fetch(`${url}/gadgets/map-reader-guard/${formData.reader_id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
                },
                body: JSON.stringify({associated_users:selectedUserList}),
            });
            if (response.ok) {
            ...
            <div className="bg-white p-5">
                ...
                <div className="px-4 py-5 sm:p-6">
                    <div className="flex flex-col space-y-2">
                        <label htmlFor="user_id" className="text-sm font-medium text-gray-700">{t("Users")}</label>
                        {/* <select
                        ...
                        >
                         ...
                         <MultipleSelectDropdown
                            ...
                            onChange={handelUserChnage}
                        />
                        {errors.user_id && <div className="text-red-500 text-xs">{errors.user_id}</div>}

                        <label htmlFor="reader_id" className="text-sm font-medium text-gray-700">{t("Reader")}</label>
                        <select
