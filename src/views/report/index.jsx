...
const Report = () => {
    ...
    const handleView = async () => {
        ...
        if (reportType === "visitor") {
            ...
            else if (selectedVisitorReportType === "Blacklisted") {
                fetchurl = `${url}/reports/visitor?download=false&start_date=${startDate}&end_date=${endDate}&field=is_blacklisted&value=True`;
            }
            else if (selectedVisitorReportType === "visitor-type") {
                fetchurl = `${url}/reports/visitor?download=false&start_date=${startDate}&end_date=${endDate}&field=visitor_type&value=${visitorType}`;
            }
            else if (selectedVisitorReportType === "visitingtozone") {
        ...
        if (reportType === "pass_report") {
            fetchurl = `${url}/reports/pass?download=false&start_date=${startDate}&end_date=${endDate}`;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (reportType === "visitor") {
                    ...
                    if (selectedVisitorReportType === "Blacklisted") {
                        simplifiedData = json.map((item) => ({
                            image: `data:image/jpeg;base64,${item.image}`,
                            first_name: item.first_name,
                            last_name: item.last_name,
                    ...
                    if (selectedVisitorReportType === "visitor-type") {
                        simplifiedData = json.map((item) => ({
                            image: `data:image/jpeg;base64,${item.image}`,
                            first_name: item.first_name,
                            last_name: item.last_name,
                    ...
                    if (selectedVisitorReportType === "visitingtozone") {
                        simplifiedData = json.map((item) => ({
                            ...
                            zone: item.zone_name,
                            reader_type: item.reader_type,
                            Authorized: item.is_authorized ? "Yes" : "No",
                            blacklisted: item.is_blacklisted ? "Yes" : "No",
                ...
                if (reportType === "pass_report") {
                    simplifiedData = json.map((item) => ({
                        image: `data:image/jpeg;base64,${item.visitor.image}`,
                        visitor_name: (item.visitor.first_name +" "+ item.visitor.last_name),
                        key: item.key_name,
                        whom_to_visit: item.whom_to_visit,
    ...
    const handleDownload = async () => {
        ...
        if (reportType === "visitor") {
            ...
            if (selectedVisitorReportType === "Blacklisted") {
                fetchurl = `${url}/reports/visitor?download=true&start_date=${startDate}&end_date=${endDate}&field=is_blacklisted&value=True`;
                filename = `visitor-visiting-report-${formattedDate}.pdf`;
            }
            if (selectedVisitorReportType === "visitor-type") {
                fetchurl = `${url}/reports/visitor?download=true&start_date=${startDate}&end_date=${endDate}&field=visitor_type&value=${visitorType}`;
                filename = `visitor-visiting-report-${formattedDate}.pdf`;
            }
            if (selectedVisitorReportType === "visitingtozone") {
        ...
        if (reportType === "pass_report") {
            fetchurl = `${url}/reports/pass?download=true&start_date=${startDate}&end_date=${endDate}`;
            filename = `Pass-report-${formattedDate}.pdf`;
        }
    ...
    const handelVisitorTypeChange = (e) => {
        setVisitorType(e.target.value);
        if (errors.visitorType) {
            setErrors((prev) => ({ ...prev, visitorType: "" }));
        }
    };
                        ...
                        <>
                            <div>
                                ...
                                >
                                    ...
                                    <option value="visitorsvisiting">Visitor Report</option>
                            ...
                            {selectedVisitorReportType === "Blacklisted" && (
                                <>
                                    <div>
                                        <input
                                            type="datetime-local"
                            ...
                            {selectedVisitorReportType === "visitor-type" && (
                                <>
                                    <div>
                                        <input
                                            type="datetime-local"
                                    ...
                                    <div>
                                        <select
                                            className="appearance-none border border-customGreen rounded-3xl bg-white py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-green-700"
                                            onChange={handelVisitorTypeChange}
                                            value={visitorType}
                                        >
                                            <option value="">Select Report</option>
                                        ...
                                        {errors.visitorType && (
                                            <p className="text-red-500 text-xs italic">{errors.visitorType}</p>
                                        )}
                                    </div>
                                </>
                            )}
