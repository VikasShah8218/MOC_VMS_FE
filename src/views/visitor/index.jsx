...
const Visitor = () => {
  ...
  const [searchByFace,setSearchByFace] = useState();
  ...
  const getSimilarFace = async (base64Image) => {
    setIsLoading(true);
    // const queryString = new URLSearchParams(searchParams).toString();
    console.log("Apne Wala HAu ")
    // console.log(searchByFace)
    try {
      const response = await fetch(`${url}/visitor/find-similar-face`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  return (
    <div>
      <Visitors visitors={visitorData} fetchData={fetchData}  isLoading={isLoading} onActionClick={handleActionClick} searchParams={searchParams} setSearchParams={setSearchParams} totalVisitors={totalVisitors} setSearchByFace={setSearchByFace} getSimilarFace={getSimilarFace} />
